...
class RerankMetric(BaseMetric):
    ...
    def _calculate_ndcg(self, predicted: List[str], relevant: List[str], k: Optional[int] = None) -> float:
    ...
    def _calculate_average_precision(self, predicted: List[str], relevant: List[str]) -> float:
    ...
    def _calculate_reciprocal_rank(self, predicted: List[str], relevant: List[str]) -> float:
    ...
    async def _process_rerank_cases(
        self, test_cases: List[LLMCase], callbacks: Optional[Callbacks] = None,
    ) -> List[Dict]:
        """处理所有rerank案例"""
        ...
        try:
            ...
            # 批量处理有效案例
            if valid_cases:
                batch_results = await self._batch_process_cases(valid_cases, callbacks=grp_cb)
                # 将批量结果插入到正确位置
            ...
            # 计算统计信息
            successful_count = len([r for r in results if r.get("rerank_success", False)])
            skipped_count = len([r for r in results if r.get("rerank_skipped", False)])
            failed_count = len([r for r in results if r.get("rerank_failed", False) or "error" in r])
        ...
        for i in range(0, len(valid_cases), batch_size):
            ...
            for original_index, task in batch_tasks:
                try:
                    ...
                    logger.warning(f"Failed to process case at index {original_index}: {str(e)}")
    ...
    def _set_embedding_results(
        ...
        for k in self.k_values:
            ...
            row_data[f"embedding_top{k}"] = embed_quote_ids[:top_k] if embed_quote_ids else []
    ...
    async def _set_rerank_results(
        ...
        if not retrieval_questions or not embed_quote_ids:
            logger.warning(f"Invalid input for rerank: query='{query}', no documents or ids")
    ...
    async def _compute(
        ...
        **kwargs: Any,
        ...
        # 初始化指标存储
        mrr_scores: Dict[str, List[float]] = {col: [] for col in self.columns_to_evaluate}
        ndcg_scores: Dict[str, List[float]] = {col: [] for col in self.columns_to_evaluate}
        map_scores: Dict[str, List[float]] = {col: [] for col in self.columns_to_evaluate}
        ...
        for result_index, result in enumerate(rerank_results):
            ...
            for column in self.columns_to_evaluate:
                try:
                    ...
                    # 计算各项指标
                    mrr_score = self._calculate_reciprocal_rank(column_refs, expected_dataids)
                    mrr_scores[column].append(mrr_score)
                    
                    # 动态计算NDCG的k值，最大不超过15
                    k_value = min(len(column_refs), 15)
                    ndcg_score = self._calculate_ndcg(column_refs, expected_dataids, k_value)
                    ndcg_scores[column].append(ndcg_score)
                    
                    # 计算平均精度
                    ap_score = self._calculate_average_precision(column_refs, expected_dataids)
        ...
        for col in self.columns_to_evaluate:
            ...
            if column_stats[col]["total_count"] > 0:
                results[f"{col}_precision"] = column_stats[col]["hit_count"] / column_stats[col]["total_count"]
                # 添加错误率统计
                if column_stats[col]["error_count"] > 0:
                    results[f"{col}_error_rate"] = column_stats[col]["error_count"] / column_stats[col]["total_count"]
        ...
        rerank_success_count = sum(1 for r in rerank_results if r.get("rerank_success", False))
        rerank_failed_count = sum(1 for r in rerank_results if r.get("rerank_failed", False))
        if rerank_success_count + rerank_failed_count > 0:
            success_rate = rerank_success_count / (rerank_success_count + rerank_failed_count)
            reason_parts.append(f"Rerank成功率: {success_rate:.3f} ({rerank_success_count}/{rerank_success_count + rerank_failed_count})")
        ...
        if "overall_mrr" in results:
            reason_parts.extend([
                ...
                f"NDCG={results['overall_ndcg']:.3f}" if "overall_ndcg" in results else "",
                f"MAP={results['overall_map']:.3f}" if "overall_map" in results else ""
            ])
