...
class PrivateReranker(BaseModel, BaseReranker):
    ...
    def _parse_api_response(self, response_data: Dict[str, Any], documents: List[str]) -> List[Dict[str, Any]]:
        """
        解析 API 响应数据。
        {
        ...
        try:
            ...
            for i, ranking in enumerate(rankings):
                ...
                processed_ranking = {
                    ...
                    "document": ranking.get("document", documents[ranking.get("index", i)]),
                }
    ...
    async def rerank(
        ...
        **kwargs: Any,
        ...
        # 执行重排序
        rankings, usage = await self._call_rerank_api(query, documents, top_k=top_k, **kwargs)
