...
class FineTuneDataSynthesizer(BaseSynthesizer):
    ...
    def __init__(self,
        ...
        # 根据配置选择执行器
        if use_process_pool:
            self.executor = ProcessPoolExecutor(max_workers=min(max_workers, mp.cpu_count()))
        else:
    ...
    def _build_precomputed_data_optimized(self, items: List[Any], collection_ids: Set[str]) -> PrecomputedData:
        """预计算数据结构构建 - 减少内存分配和提升速度"""
        # 预分配字典容量，减少动态扩容
        estimated_questions = len(items) * 4  # 估算问题数量
        all_questions: List[str] = []
        all_questions.reserve(estimated_questions) if hasattr(all_questions, 'reserve') else None
        
        question_to_items: Dict[str, Set[str]] = {}  # 使用集合避免重复
        question_to_collection: Dict[str, str] = {}
        collection_questions: Dict[str, Set[str]] = {cid: set() for cid in collection_ids}
        ...
        # 转换为预期格式（集合转列表）
        question_to_items_list = {q: list(items_set) for q, items_set in question_to_items.items()}
        collection_questions_list = {cid: list(questions_set) for cid, questions_set in collection_questions.items()}
    ...
    def _build_precomputed_data(self, items: List[Any], collection_ids: Set[str]) -> PrecomputedData:
        """构建预计算数据结构"""
        # 检查缓存
        cache_key = self._get_question_hash(str(len(items)) + str(sorted(collection_ids)))
    ...
    def _sample_negatives_optimized(
        ...
        with worker_lock:
            same_collection_questions = precomputed.collection_questions.get(collection_id, [])
            same_collection_candidates = []
            for question in same_collection_questions:
                if (question != exclude_q1 and
                    question != exclude_q2 and
                    item_id not in precomputed.question_to_items.get(question, set())):
            ...
            for cid, questions in precomputed.collection_questions.items():
                ...
                for question in questions:
                    if (question not in exclude_set and
                        item_id not in precomputed.question_to_items.get(question, set())):
        ...
        # 计算需要采样的数量
        total_candidates = len(same_collection_candidates) + len(other_collection_candidates)
        if total_candidates == 0:
            return []
        
        num_negatives = max(
            min_negative_samples,
            min(max_negative_samples, int(total_candidates * self.negative_sample_ratio))
        )
        
        # 计算同库样本比例 - 使用缓存的统计值
        current_collection_percentage = collection_stats[collection_id]["percentage"] / 100
        ...
        if total_candidates > 1000 and hasattr(rng, 'random'):
            ...
            if same_collection_count > 0 and same_collection_candidates:
                actual_same_count = min(same_collection_count, len(same_collection_candidates))
                if actual_same_count <= len(same_collection_candidates):
                    negatives.extend(rng.sample(same_collection_candidates, actual_same_count))
                else:
            ...
            if needed_other > 0 and other_collection_candidates:
                ...
                if actual_other_count <= len(other_collection_candidates):
                    negatives.extend(rng.sample(other_collection_candidates, actual_other_count))
                else:
        ...
        if len(negatives) < num_negatives:
            ...
            remaining_candidates = [
                q for q in (same_collection_candidates + other_collection_candidates)
                if q not in negatives_set
            ]
            
            if remaining_candidates:
                if len(remaining_candidates) >= remaining_needed:
                    additional_samples = rng.sample(remaining_candidates, remaining_needed)
                else:
    ...
    def _compute_collection_stats_optimized(self, items: List[Any]) -> Tuple[Dict[str, Dict[str, Any]], int]:
        """统计信息计算"""
        collection_stats: Dict[str, Dict[str, Any]] = {}
    ...
    async def _build_fine_tune_data(
        ...
        if min_negative_samples < 0 or max_negative_samples < 0:
            raise ValueError(f"Negative sampling parameters must be non-negative: min={min_negative_samples}, max={max_negative_samples}")

        if min_negative_samples > max_negative_samples:
            raise ValueError(f"min_negative_samples ({min_negative_samples}) cannot be greater than max_negative_samples ({max_negative_samples})")
        ...
        # 1. 统计信息计算
        collection_stats, total_index_pairs = self._compute_collection_stats_optimized(items)
        logger.info(f"统计完成：{len(collection_stats)} 个知识库，{total_index_pairs} 个问题对")
        ...
        logger.info(f"预构建映射表完成，耗时: {precomputed_time:.3f}s，{len(precomputed.all_questions)} 个唯一问题") 

        # 3. 并发构建样本
        all_samples: List[Any] = []
        collection_samples: Dict[str, List[Any]] = {collection_id: [] for collection_id in collection_stats}
        ...
        logger.info(f"样本生成完成：{len(all_samples)} 个样本，耗时: {processing_time:.3f}s，速度: {len(all_samples)/processing_time:.1f} 样本/秒")
        
        # 基础统计信息
        base_statistics = {
            "unique_questions": len(precomputed.all_questions),
            "total_index_pairs": total_index_pairs,
            "average_negatives_per_sample": sum(len(sample.negatives) for sample in all_samples) / max(len(all_samples), 1),
            "samples_per_item": len(all_samples) / max(len(items), 1),
            "processing_speed": len(all_samples) / max(processing_time, 0.001),
            "concurrent_processing": len(items) > self.chunk_size and self.max_workers > 1
        }
        ...
        for collection_id, stats in collection_stats.items():
            ...
            collection_statistics[collection_id] = {
                ...
                "samples_per_item": collection_samples_count / max(stats["data_count"], 1)
            }
        
        # 合并统计信息
        total_actual_samples = sum(stats["actual_samples"] for stats in collection_statistics.values())
        merged_statistics = {
            **base_statistics,
        ...
        return {
            ...
            "negative_samples": sum(len(sample.negatives) for sample in all_samples),  # 总负样本数量
    ...
    def optimize_for_dataset_size(self, dataset_size: int) -> None:
        ...
        logger.info(f"根据数据集大小({dataset_size})优化参数: chunk_size={self.chunk_size}, max_workers={self.max_workers}")
