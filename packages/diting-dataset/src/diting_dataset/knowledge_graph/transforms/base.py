...
class Extractor(BaseGraphTransformation):
    ...
    def generate_execution_plan(
        ...
        async def apply_extract(node: Node) -> None:
...
class LLMBasedExtractor(Extractor):
    ...
    def split_text_by_token_limit(self, text: str, max_token_limit: int) -> t.List[str]:
...
class Splitter(BaseGraphTransformation):
    ...
    async def transform(
        ...
        all_nodes: t.List[Node] = []
        all_relationships: t.List[Relationship] = []
    ...
    def generate_execution_plan(
        ...
        async def apply_split(node: Node) -> None:
            nodes, relationships = await self.split(node)
...
class NodeFilter(BaseGraphTransformation):
    async def transform(self, kg: KnowledgeGraph) -> KnowledgeGraph:
        filtered = self.filter(kg)
        for node in filtered.nodes:
            if await self.custom_filter(node, filtered):
...
@dataclass
class LLMBasedNodeFilter(NodeFilter):
    llm: BaseLLM = field(default_factory=llm_factory)
