...
class CustomNodeFilter(LLMBasedNodeFilter):
    ...
    scoring_prompt: PydanticPrompt[QuestionPotentialInput, QuestionPotentialOutput] = (
        field(default_factory=QuestionPotentialPrompt)
    )
    min_score: int = 2
    ...
    async def custom_filter(self, node: Node, kg: KnowledgeGraph) -> bool:
        ...
        response = await self.scoring_prompt.generate(llm=self.llm, data=prompt_input)
