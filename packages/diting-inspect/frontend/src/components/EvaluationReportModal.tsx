...
const EvaluationResult: React.FC<EvaluationResultProps> = ({ result }) => {
	return (
		<div className="border p-4 mb-4 rounded shadow">
			...
			<p>
				<strong>Score:</strong> {result.score !== null ? result.score : "N/A"}
			</p>
			{result.error && (
			...
			{result.run_logs &&
				Object.keys(result.run_logs).length > 0 && ( // 检查 run_logs 是否存在且有内容
					<div>
						<h4 className="font-semibold">Run Logs:</h4>
						<pre>{JSON.stringify(result.run_logs, null, 2)}</pre>{" "}
						{/* 以 JSON 格式展示 */}
					</div>
				)}
...
const EvaluationReportModal: React.FC<EvaluationReportModalProps> = ({
	isOpen,
	onClose,
	evaluation,
}) => {
	...
	return (
		<div className="fixed inset-0 flex items-center justify-center z-50">
			...
			></div>
			<div
				className="bg-white rounded-lg shadow-lg p-6 z-10 w-full max-w-3xl flex flex-col"
