...
async def build_fine_tune_data(
    request: FineTuneDataRequest, http_request: Request
) -> FineTuneDataResponse:
    """
    构建微调数据接口
    ...
    logger.info(
        "Building fine-tune data", request_id=request_id, total_items=len(request.items)
    )
    ...
    try:
        ...
        logger.error(
            "Failed to build fine-tune data", request_id=request_id, error=str(e)
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
