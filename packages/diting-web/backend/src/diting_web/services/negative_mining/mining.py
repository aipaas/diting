...
async def mine_negatives(
    ...
    - callbacks: 可选的回调函数列表，用于收集 token 使用情况

    返回：
    - (增强后的训练数据列表, token使用情况列表)
    """
    # 1) 构造语料池与查询集
    corpus: List[str] = []
    queries: List[str] = []
    ...
    for i in range(0, len(corpus), embedding_batch_size):
        ...
        batch_embeddings = await embedding_model.aembed_documents(batch_corpus, callbacks=callbacks)
    ...
    for i in range(0, len(queries), embedding_batch_size):
        ...
        batch_embeddings = await embedding_model.aembed_documents(batch_queries, callbacks=callbacks)
    ...
    if callbacks:
        for callback in callbacks:
            if hasattr(callback, 'usages'):
                usages.extend(callback.usages)
    
    return enhanced_data, usages
