"""Model configuration helpers for diting-core SDK.

Provides utility functions to create LLM and embedding models from configuration dictionaries.
These helpers bridge the gap between diting-web's database models and diting-core's SDK.
"""
...
def create_llm_from_config(llm_config: dict[str, Any]) -> BaseLLM:
    ...
    Uses diting-server's resolve_model_config for URL normalization and default values,
    providing the same behavior as EvaluationRunner.
    
    Args:
        llm_config: LLM configuration dictionary with fields:
            - model_name or name (str): Model name (e.g., "gpt-4", "gpt-3.5-turbo")
    ...
    """
    # Support both 'model_name' and 'name' fields for compatibility
    model_name = llm_config.get("model_name") or llm_config.get("name", "gpt-4o-mini")
    base_url = llm_config.get("base_url")
    api_key = llm_config.get("api_key")
    timeout = llm_config.get("timeout", 60.0)
    
    # Use resolve_model_config for URL normalization and env var fallback (same as EvaluationRunner)
    ...
    logger.info(
        ...
        has_api_key=bool(resolved_config.get("api_key")),
        timeout=timeout,
    ...
    try:
        llm = llm_factory(
            **resolved_config,  # Use resolved config (includes model, base_url, api_key)
            timeout=timeout,
            **kwargs,
        )
        logger.info("LLM created successfully", model=resolved_config["model"])
        return llm
...
def create_embedding_from_config(embedding_config: dict[str, Any]) -> BaseEmbeddings:
    ...
    Args:
        embedding_config: Embedding configuration dictionary with fields:
            - model_name or name (str): Model name (e.g., "text-embedding-ada-002", "bge-m3")
            - base_url (str, optional): API base URL
            - api_key (str, optional): API key
            - timeout (float, optional): Request timeout in seconds
    ...
    """
    # Support both 'model_name' and 'name' fields for compatibility
    model_name = embedding_config.get("model_name") or embedding_config.get("name", "bge-m3")
    ...
    try:
        ...
        logger.info("Embedding created successfully", model=resolved_config["model"])
        return embedding
    except Exception as e:
