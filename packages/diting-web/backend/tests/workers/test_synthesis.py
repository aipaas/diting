...
class TestRunSynthesis:
    ...
    @pytest.mark.asyncio
    @patch("diting_web.workers.synthesis.SynthesisRunner.create_synthesizer")
    async def test_run_synthesis_all_generations_fail(
        self, mock_create_synthesizer, mock_corpus_data
    ):
        """Test that when all generations fail, an empty list is returned."""
        # Arrange
        mock_synthesizer = MagicMock()
        
        # All generations fail
        mock_synthesizer.apply = AsyncMock(side_effect=Exception("Connection error"))
        mock_create_synthesizer.return_value = mock_synthesizer
        ...
        assert mock_synthesizer.apply.call_count == 3
