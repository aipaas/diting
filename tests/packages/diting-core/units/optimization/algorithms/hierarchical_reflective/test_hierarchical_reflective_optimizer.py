...
class TestHierarchicalReflectiveOptimizer:
    ...
    @pytest.mark.asyncio
    @patch(
        "diting_core.optimization.algorithms.hierarchical_reflective.optimizer.evaluate_prompt_with_detail",
    ...
    def test_calculate_improvement(self):
        ...
        improvement = optimizer._calculate_improvement(0.8, 0.7)
        assert pytest.approx(improvement, abs=1e-4) == (0.8 - 0.7) / 0.7

        # Test no improvement
        improvement = optimizer._calculate_improvement(0.7, 0.7)
        assert improvement == 0.0

        # Test regression
        improvement = optimizer._calculate_improvement(0.6, 0.7)
        assert improvement < 0

        # Test with zero baseline
        improvement = optimizer._calculate_improvement(0.5, 0.0)
        assert improvement == 0
