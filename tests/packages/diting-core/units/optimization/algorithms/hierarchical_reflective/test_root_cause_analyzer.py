...
from diting_core.optimization.algorithms.hierarchical_reflective.root_cause_analyzer import (
    HierarchicalRootCauseAnalyzer,
)
from diting_core.optimization.algorithms.hierarchical_reflective.types import (
    FailureMode,
    BatchAnalysis,
...
class TestHierarchicalRootCauseAnalyzer:
    ...
    def call_model_fn(self, mock_llm_adapter):
        ...
        async def _call_model_fn(messages, seed, response_model):
    ...
    async def test_analyze_batch_async(self, analyzer, sample_test_results):
        ...
        batch_analysis = await analyzer._analyze_batch_async(
            test_results=sample_test_results,
            batch_number=1,
            batch_start=0,
            batch_end=5,
    ...
    async def test_synthesize_batch_analyses_async(self, analyzer, sample_test_results):
        ...
        synthesis = await analyzer._synthesize_batch_analyses_async(
            test_results=sample_test_results,
    ...
    async def test_analyze_async_full_flow(self, analyzer, sample_test_results):
        ...
        result = await analyzer.analyze_async(sample_test_results)
    ...
    async def test_analyze_async_with_missing_reasons(self, analyzer):
        ...
        with pytest.raises(ValueError, match="must include 'reason' fields"):
            await analyzer.analyze_async(test_results_no_reasons)
    ...
    async def test_analyze_async_single_batch(self, call_model_fn):
        ...
        result = await analyzer.analyze_async(test_results)
    ...
    def test_analyze_sync_wrapper(self, analyzer, sample_test_results):
        """Test synchronous wrapper works correctly"""
        result = analyzer.analyze(sample_test_results)

        # Should return same structure as async version
        assert isinstance(result, HierarchicalRootCauseAnalysis)
        assert result.total_test_cases == 10
        assert len(result.unified_failure_modes) > 0
    ...
    async def test_concurrent_batch_processing(self, mock_llm_adapter):
        ...
        async def counting_call_model_fn(messages, seed, response_model):
            nonlocal call_count
            call_count += 1
