...
from diting_core.optimization.algorithms.parameter import (
    ParameterOptimizer,
...
test_helpers_path = Path(__file__).parent.parent.parent
...
class TestParameterOptimizer:
    ...
    def test_optimizer_initialization(self):
        ...
        optimizer = ParameterOptimizer(
            default_n_trials=20, seed=42, local_search_ratio=0.3
        )

        assert optimizer.default_n_trials == 20
    ...
    async def test_optimize_requires_parameter_space(self):
        ...
        optimizer = ParameterOptimizer(default_n_trials=5)
    ...
    @pytest.mark.asyncio
    @patch(
        "diting_core.optimization.algorithms.parameter.optimizer.evaluate_prompt",
        mock_evaluate_prompt,
    )
    @patch(
        "diting_core.optimization.algorithms.parameter.optimizer.evaluate_prompt_sync",
        mock_evaluate_prompt_sync,
    )
    async def test_optimize_with_dict_parameter_space(self):
        """测试使用字典形式的 parameter_space 进行优化"""
        optimizer = ParameterOptimizer(default_n_trials=3, seed=42)
    ...
    async def test_optimize_with_parameter_search_space(self):
        ...
        optimizer = ParameterOptimizer(default_n_trials=5, seed=42)
        ...
        assert result.best_prompt.model_params is not None
        assert "temperature" in result.best_prompt.model_params
        assert "max_tokens" in result.best_prompt.model_params
    ...
    async def test_two_stage_optimization(self):
        """测试两阶段优化（全局搜索 + 局部搜索）"""
        optimizer = ParameterOptimizer(
            default_n_trials=10, seed=42, local_search_ratio=0.3
        )
        prompt = PromptConfig(user="{input}", llm=MockLLMAdapter())
        dataset = InMemoryDataset(
            name="test_dataset",
    ...
    async def test_parameter_importance_analysis(self):
        """测试参数重要性分析"""
        optimizer = ParameterOptimizer(default_n_trials=8, seed=42)
        prompt = PromptConfig(user="{input}", llm=MockLLMAdapter())
        dataset = InMemoryDataset(
            name="test_dataset",
            items=[{"id": "1", "input": "test", "expected_output": "test"}],
