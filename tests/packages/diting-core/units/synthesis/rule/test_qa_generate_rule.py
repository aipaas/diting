...
from diting_core.synthesis.rules.qa.schema import QA, QAWithScore
...
class MockQAQualityMetric(BaseMetric):
    async def _compute(
        ...
        return MetricValue(score=1, reason="test")


class TestQAGenerateRule(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        self.mock_model = MockLLM()
        self.rule = QAGenerateRule(model=self.mock_model)
        self.rule.metric = MockQAQualityMetric()  # Mock the QAQualityMetric

    async def test_apply_success(self):
        """测试正常情况下的_apply方法"""
        rule_input = {LLMCaseParams.CONTEXT.value: ["Explain AI."]}
